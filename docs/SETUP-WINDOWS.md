...
- 配置本仓库 LLM Wiki MCP 构建产物，强制 `LLM_WIKI_MCP_TOOLSET=research`，将 `platform_toolset.cli` 固定为仅允许 `llm-wiki`，并通过 Profile `.env` 的 `HERMES_BRIDGE_TOOLSETS=llm-wiki` 覆盖 Bridge 默认工具恢复；Studio research 会话只能看到 `search/read/graph` 三个只读工具，终端、文件写入及其他 MCP 不会启用。
...
Windows 系统时区应设为 `China Standard Time`（Asia/Shanghai），否则每日备份会按错误的本地时区触发。
...
备份包含 Hermes Profile 的 `state.db`、Studio 的 `hermes-web-ui.db`、记忆/技能的非凭据文件、Wiki 原文/正式页/草稿审核状态。SQLite 使用一致性快照；目录在复制前后做 SHA-256 对比。若会话或文本中检出常见明文 API Key，整次备份失败并删除临时目录。`.env`、Token、认证 JSON 和私钥类文件始终排除。
...
恢复会先验证全部文件哈希、SQLite 完整性、版本锁和目标目录边界，再暂存并替换；中途失败会恢复原文件。恢复后用启动器重新启动，并抽查一次会话、记忆、论文引用和待审核草稿。完整演练目标为 RTO 不高于 2 小时。
